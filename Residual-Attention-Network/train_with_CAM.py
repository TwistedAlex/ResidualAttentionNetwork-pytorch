...
def train(model, device, logger, epoch, train_loader, optimizer, criterion, writer, cfg):
    ...
    for sample in train_loader.datasets['train']:
        ...
        used_mask_indices = [sample['idx'].index(x) for x in sample['idx']
                             if x in train_loader.train_dataset.used_masks]
        ...
        #     break
        break
    writer.add_scalar('Loss/train/avg_cl_loss_per_epoch',
                      total_losses / (iter_i * args.batchsize),
                      epoch)
    writer.add_scalar('Loss/train/avg_ex_loss_per_epoch',
                      epoch_train_ex_loss / (iter_i * args.batchsize),
                      epoch)
...
parser.add_argument('--nepoch', type=int, default=500, help='number of iterations per epoch')
